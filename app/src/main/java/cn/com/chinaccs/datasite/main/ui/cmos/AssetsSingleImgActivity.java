...
public class AssetsSingleImgActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btnSave.setOnClickListener(affixLr);
		if (isSub) {
		...
		Log.d(App.LOG_TAG, "imgURL: " + imgURL);
	...
	private void addCacheContent() {
		List<AssetsPhotosModel> list = DAOAssetsPhoto.getInstance(context)
				.searchAssetsSubType(id,subType,false);
		if(null!=list && list.size()>0){
			if (list.get(0).getLocalImgUrl() == null || list.get(0).getLocalImgUrl().equals("")) {
				
			} else {
				File file = new File(list.get(0).getLocalImgUrl());
				...
				bm = null;
				MainApp.app.imagePath = list.get(0).getLocalImgUrl();
			}
		}
	...
	private void findViews() {
		...
		btnSave = (Button) findViewById(R.id.btn_img_save);
	}
	...
	protected void save() {
		if (MainApp.app.imagePath != null && !MainApp.app.imagePath.equals("")) {
			saveCacheInfo();
			toast("本地保存成功！");
			setResult(AssetsCameraFlowActivity.RESULT_CACHE);
			finish();
		}else{
			toast("尚未选择文件！");
		}
	...
	private void saveCacheInfo(){
		//缓存图片信息
		AssetsPhotosModel assetsPhoto = new AssetsPhotosModel();
		assetsPhoto.setDsId(id);
		assetsPhoto.setType(type);
		assetsPhoto.setSubType(subType);
		assetsPhoto.setDesc(etDes.getText().toString());
		assetsPhoto.setLocalImgUrl(MainApp.app.imagePath);
		assetsPhoto.setCreatDate(new Date());
		assetsPhoto.setUpload(false);
		DAOAssetsPhoto.getInstance(context).insertCache(assetsPhoto);
	}
