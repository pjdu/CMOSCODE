 ...
 * 该界面没有被任何地方引用
 */
public class AssetsContentModifyActivity extends Activity {
...
//	private FuncDSCommon funcBs;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.context = this;
		setContentView(R.layout.activity_assets_content_modify);
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				try {
					...
					if (!res.equals("1")) {
						...
						if(DAOAssets.getInstance(AssetsContentModifyActivity.this).searchAssetsId(id) != null){

						    assets = DAOAssets.getInstance(AssetsContentModifyActivity.this).searchAssetsId(id).get(0);
						}
...
//							cashData(array);//返回缓存数据
		                    be.putString("array", cashData(array,ps));

							Intent i = new Intent(context, AssetsModifyActivity.class);
							i.putExtras(be);
...
//					cashData(array);//返回缓存数据
//					be.putString("array", cashData(array));
	...
	public String cashData(JSONArray array,int[] ps){
		...
		try {
			...
			AssetsModel assets = DAOAssets.getInstance(AssetsContentModifyActivity.this).searchAssetsId(id).get(0);
	...
	static class BsHandler extends Handler {
		...
		private AssetsContentModifyActivity at;

		public BsHandler(Activity at) {
			this.at = (AssetsContentModifyActivity) at;
		}
