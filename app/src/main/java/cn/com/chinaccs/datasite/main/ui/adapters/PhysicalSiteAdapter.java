 ...
 */
public class PhysicalSiteAdapter extends RecyclerView.Adapter<PhysicalSiteAdapter.ViewHolder>
        implements View.OnClickListener {

    private Context mContext;
    private List<PhysicalSiteBean> mBeans;
    private OnRecyclerViewItemClickListener mOnItemClickListener = null;

    //define interface
    public static interface OnRecyclerViewItemClickListener {
        void onItemClick(View view, PhysicalSiteBean data);
    }

    public PhysicalSiteAdapter(Context context, List<PhysicalSiteBean> beans) {
        this.mContext = context;
        this.mBeans = beans;
    }

    public void setData(List<PhysicalSiteBean> items) {
        this.mBeans = items;
        notifyDataSetChanged();
    }

    public void addData(PhysicalSiteBean item) {
    ...
    @Override
    public PhysicalSiteAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_item_physical_site, parent,
                false);
        //将创建的View注册点击事件
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        PhysicalSiteBean noteModel = mBeans.get(position);
    ...
    public void onClick(View v) {
        if (mOnItemClickListener != null) {
            //注意这里使用getTag方法获取数据
            mOnItemClickListener.onItemClick(v, (PhysicalSiteBean) v.getTag());
        }
    }
