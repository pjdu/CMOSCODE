...
public class WGSTOGCJ02 {
    ...
    public Map<String, Double> gcj2wgs(double lon, double lat) {
        ...
        double latitude = (lat - (((Double) (transform(lon, lat)).get("lat"))
                .doubleValue() - lat));
        BigDecimal bdlng = new BigDecimal(longtitude);
        longtitude = bdlng.setScale(6, BigDecimal.ROUND_HALF_UP).doubleValue();
        BigDecimal bdlat = new BigDecimal(latitude);
        latitude = bdlat.setScale(6, BigDecimal.ROUND_HALF_UP).doubleValue();
        localHashMap.put("lon", longtitude);
