...
/**
 * @author Fddi c网某基站详细界面
 */
public class DSCdmaBSContentActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btsid = be.getString("btsid");
		bsc = be.getString("bsc");
		this.findViews();
		tvName.setText(name);
		this.getData();
	...
	private void buildListGroup() {
		...
		String[][] children = {
				{ getResources().getString(R.string.bs_item_biarea),
						...
						getResources().getString(R.string.bs_item_biconf) },
				{ getResources().getString(R.string.bs_item_ptzgj),
						getResources().getString(R.string.bs_item_pttransfer),
						getResources().getString(R.string.bs_item_ptele),
						getResources().getString(R.string.bs_item_ptkt),
						getResources().getString(R.string.bs_item_ptroom) },
				{ getResources().getString(R.string.bs_item_other) } };

		final int[][][] positons = { { { 1, 4 }, { 5, 27 }, { 28, 32 } },
				{ { 33, 36 }, { 37, 45 }, { 46, 63 }, { 64, 67 }, { 78, 91 } },
				{ { 68, 77 } } };
	...
	private void updateData() {
		...
		be.putInt("datatype", DataSiteStart.TYPE_BASESTATION);
		be.putString("id", id);
...
//				startActivityForResult(i, 0);
	...
	private OnClickListener btnLr = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.btn_img:
				...
				i.putExtra("type", DataSiteStart.TYPE_BASESTATION);
				i.putExtra("id", id);
				startActivity(i);
				break;
			case R.id.btn_datac:
	...
	static class BsHandler extends Handler {
		...
		private DSCdmaBSContentActivity at;

		public BsHandler(Activity at) {
			this.at = (DSCdmaBSContentActivity) at;
		}
	...
	private void cellConn(Handler handler) {
		...
		Message msg = null;
		StringBuffer url = new StringBuffer(DataSiteStart.HTTP_SERVER_URL);
		String sign = App.signMD5(DataSiteStart.HTTP_KEYSTORE + userId + btsid
				+ bsc);
		try {
			String btsid1 = URLEncoder.encode(btsid, App.ENCODE_UTF8);
			String bsc1 = URLEncoder.encode(bsc, App.ENCODE_UTF8);
			url.append("DSCDMABSCEELS.do?userid=").append(userId)
					.append("&btsid=").append(btsid1).append("&bsc=")
					.append(bsc1).append("&sign=").append(sign);
