 ...
 * 
 */
public class FuncAgentUserInfo {

	private Context context;

	public FuncAgentUserInfo(Context context) {
		this.context = context;
	}

	public void getData(final OnGetDataFinishedListener lr, final String agentUserId) {
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					String sign = App.signMD5(DataSiteStart.HTTP_KEYSTORE + userid + agentUserId);
					String agentUser = URLEncoder.encode(agentUserId, App.ENCODE_UTF8);
					StringBuffer url = new StringBuffer(
							DataSiteStart.HTTP_SERVER_URL);
					url.append("AgentUserInfo.do?userid=").append(userid)
							.append("&agentUserId=").append(agentUser)
