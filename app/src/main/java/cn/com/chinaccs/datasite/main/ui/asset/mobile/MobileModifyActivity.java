 ...
 *
 * Last Modified on 2017-08-17 10:15:52.
 */
 ...
 */
public class MobileModifyActivity extends BaseActivity {
        ...
        try {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (DAOAssets.getInstance(MobileModifyActivity.this).searchAssetsId(id) == null) {
                        DAOAssets.getInstance(MobileModifyActivity.this).add(assetsInfo);
                    } else {
                        AssetsModel assets = DAOAssets.getInstance(MobileModifyActivity.this).searchAssetsId(id).get(0);
                        try {
                            if (ps[0] == 1 && ps[1] == 36) {
                                assetsInfo.setZdWlzmc(array.getString(0));
                                assetsInfo.setZdWlzbh(array.getString(1));
                            ...
                            DAOAssets.getInstance(MobileModifyActivity.this).update(assets);
                        } catch (JSONException e) {
                            // TODO Auto-generated catch block
                            e.printStackTrace();
                        }
    ...
    private void buildForms() {
        ...
        try {
            ...
            for (int i = 0; i < array.length(); i++) {
                ...
                if (index >= start && index <= end) {
                    ...
                    if (vtype.equals("3")) {
                        ...
                        if (data.getString(0).indexOf("编号") > 0) {
                            dm.setType("qrcode");
                        }
            ...
            for (int i = 0; i < listDm.size(); i++) {
                ...
                } else if (type.equals(DataModify.TYPE_QRCODE)) {
                    factory.addQrCodeView(attr, layout, listEt, i);
                }
            }
            layout.invalidate();
        } catch (Exception e) {
                ...
                if (data != null && resultCode == RESULT_OK) {
                    ...
                    handler.post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            listEt.get(index).setText(num);
                            Toast.makeText(context, "固定资产编号:" + num, Toast.LENGTH_LONG)
                                    .show();
                        }
                    });
                }
