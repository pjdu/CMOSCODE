 ...
 *
 * Last Modified on 2017-10-27 10:08:09.
 */
 ...
 * 光网络资源详细信息
 */
@EActivity(R.layout.activity_asset_mobile_con)
public class MobileConItemInfoActivity extends BaseActivity {
            ...
            case REQUEST_SERIAL_CODE_SCANNING:
                if (data != null && resultCode == RESULT_OK) {
                    ...
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            serialId = bundle.getString("result");
                            ALog.i(serialId);
                            String resScanSn = mSerialIdEt.getText().toString().trim();
                            if (resScanSn.equals("")) {
                                mSerialIdEt.setText(serialId);
                            } else {
                                if (!resScanSn.equals(serialId)) {
