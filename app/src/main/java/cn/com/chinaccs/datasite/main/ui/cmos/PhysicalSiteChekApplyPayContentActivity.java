...
public class PhysicalSiteChekApplyPayContentActivity extends BaseActivity implements OnClickListener {
    ...
    //基站下一阶段的类型 2站址验收 3入网申请 4验收交付 对应upload中的type，接口目前不确认
    private int type;
    //新增的失败和成功按钮
    private LinearLayout linearLayout;
    private LinearLayout linearLayout_assets_check;
    private Button success;
    private Button fail;
    private int ids;
    private int typeZZYS;//2站址验收不通过 3站址验收通过
    private int typeRWSQ;//4申请不通过 5入网申请通过
    private int YSJF;//0验收交付通过
    private String TAG = PhysicalSiteChekApplyPayContentActivity.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.context = this;
        setContentView(R.layout.activity_physical_site_content);
        initToolbar("详细信息");
//		funcBs = new FuncDSCommon(context);
        ...
        if (Integer.valueOf(type) != null) {
            ...
            linearLayout_assets_check.setVisibility(View.GONE);
            if (type == 2) {
                success.setText("同意验收");
                fail.setText("不同意验收");
            } else if (type == 3) {
                success.setText("同意入网");
                fail.setText("不同意入网");
            } else {
                success.setText("同意交付");
                fail.setText("不同意交付");
            }
        }
        tvName.setText(name);
...
//		funcBs = null;
    ...
    private void findView() {
        ...
        btnImg = (FloatingActionButton) findViewById(R.id.btn_img_assets);
        btnDatac = (Button) findViewById(R.id.btn_datac_assets);
        tvState = (TextView) findViewById(R.id.txt_state_assets);
        //新镇按钮
        success = (Button) findViewById(R.id.btn_physicalsite_success);
        fail = (Button) findViewById(R.id.btn_physicalsite_fail);
        success.setOnClickListener(this);
        fail.setOnClickListener(this);

        linearLayout = (LinearLayout) findViewById(R.id.linear_btn_success_fail);
        linearLayout_assets_check = (LinearLayout) findViewById(R.id.linear_btn_assets_check);
    }
    ...
    private void buildList() {
        ...
        final int[][] positons = {{1, 36}, {37, 63}, {64, 102}, {103, 107}, {108, 115},
...
//                }
                ...
                try {
                    ...
                    if (!res.equals("1")) {
                        ...
                        if (DAOAssets.getInstance(PhysicalSiteChekApplyPayContentActivity.this).searchAssetsId(id) != null) {
                            assets = DAOAssets.getInstance(PhysicalSiteChekApplyPayContentActivity.this).searchAssetsId(id).get(0);
                        }
...
//							cashData(array);//返回缓存数据
                            ...
                            Intent i = new Intent(context, AssetsItemActivity.class);
                            i.putExtras(be);
...
//					cashData(array,new int[]{1,240});//返回缓存数据
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_physicalsite_success:
                if (type == 2) {
                    typeZZYS = 3;
                    upLoad(typeZZYS, ids);
                } else if (type == 3) {
                ...
                finish();
                break;
            case R.id.btn_physicalsite_fail:
                if (type == 2) {
                    typeZZYS = 2;
                    upLoad(typeZZYS, ids);
                } else if (type == 3) {
    ...
    private class ReadCacheTask extends AsyncTask<String, Void, List<AssetsModel>> {
        ...
        protected List<AssetsModel> doInBackground(String... params) {
            return DAOAssets.getInstance(PhysicalSiteChekApplyPayContentActivity.this).searchAssetsId(params[0]);
        }
    ...
    public String cashData(JSONArray array, int[] ps) {
        ...
        try {
            ...
            AssetsModel assets = DAOAssets.getInstance(PhysicalSiteChekApplyPayContentActivity.this).searchAssetsId(id).get(0);
    ...
    private void conn(Handler hr) {
        ...
        url.append("AssetsContent.do?userid=").append(userId).append("&id=")
        ...
        try {
            ...
            jsonRes = resJson;
            Log.d(App.LOG_TAG, jsonRes.toString());
    ...
    static class BsHandler extends Handler {
        ...
        private PhysicalSiteChekApplyPayContentActivity at;

        public BsHandler(Activity at) {
            this.at = (PhysicalSiteChekApplyPayContentActivity) at;
        }
